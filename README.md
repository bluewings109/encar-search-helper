...
화면 우측에는 **필터 패널**을 제공해, 용도변경이력·단순수리 여부(각각 "있음/없음/정보없음")와 신차대비 비율(예: "60% 이하만 보기")로 매물을 선택적으로 숨기거나 보이게 할 수 있습니다(모든 조건을 동시에 만족하는 매물만 표시). 설정은 브라우저에 저장되어 다음 방문에도 유지됩니다.

## 동작 원리

엔카 웹사이트가 내부적으로 사용하는 공개 API를 활용합니다.

1. `GET https://api.encar.com/v1/readside/vehicle/{listId}` — 목록의 매물 ID를 실제 `vehicleId`/`vehicleNo`로 변환, 신차대비 비율 계산에 쓰는 `category.originPrice`(기본가)·`advertisement.price`(판매가)·`options.choice`(선택 옵션 코드)도 포함
...
4. `GET https://api.encar.com/v1/readside/vehicles/car/{vehicleId}/options/choice` — 옵션 코드별 가격표(신차대비 비율 계산 시 선택 옵션 가격 합산에 사용)
...
신차대비 현재가 비율은 `현재 판매가(advertisement.price) ÷ 신차가(category.originPrice + 선택 옵션가 합계) × 100`으로 계산합니다. 예를 들어 87.9%면 "이 매물이 신차가의 87.9% 가격"이라는 뜻이고(감가된 비율이 아니라 남아있는 가격 비율입니다), 값이 낮을수록 신차 대비 많이 하락한 가격입니다. 엔카 상세페이지의 "신차대비" 표시는 자체 시세표 매칭에 실패하면 "-%"로 나오는 경우가 많아, 그 값을 그대로 가져오는 대신 확장에서 독자적으로 계산합니다(엔카가 화면에 표시하는 값과 정확히 일치하지 않을 수 있습니다). 기본가나 판매가 정보가 없는 매물은 "정보없음"으로 표시됩니다.
...
신차대비 비율 필터는 최댓값만 입력받습니다(예: 60 입력 시 신차가의 60% 이하 가격인 매물만 표시). "정보없음 포함" 체크를 해제하면 비율을 계산할 수 없는 매물은 목록에서 제외됩니다.

필터 패널에서 표시 조건을 변경하면, 아직 조회하지 않은 카드는 즉시 조회하고(스크롤을 기다리지 않음) 이미 조회한 카드는 즉시 표시/숨김을 다시 적용합니다. 필터 항목별 설정은 각각 `localStorage`(`eh-usage-change-filter`, `eh-simple-repair-filter`, `eh-max-price-ratio-filter` 키)에 저장됩니다.
...
src/filter.js          용도변경이력·단순수리·신차대비 비율 필터 패널 UI 및 설정 저장/알림
