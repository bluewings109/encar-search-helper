...
- 검색 조건은 애드온 **구성(Configuration)** 화면에서 직접 지정합니다. 별도 파일 편집이 필요 없습니다.
- 조건 지정은 encar.com(PC/모바일 웹)에서 원하는 대로 검색/필터링한 뒤 **주소창 URL을 그대로 복사**하는 방식이라 별도 문법을 배울 필요가 없습니다.
- 신차대비 비율 상한, 용도변경이력/단순수리 제외처럼 encar 검색 UI에 없는 조건도 구성 화면에서 검색 항목마다 추가로 지정할 수 있습니다.
...
4. 설치 후 시작하기 전에 아래 3단계로 구성부터 준비하세요.

## 3. 구성(Configuration) 준비

애드온 설치 후 **구성** 탭에서 설정합니다.

1. 1단계에서 얻은 `bot_token`, `chat_id`를 채웁니다.
2. `searches`에 감시할 조건을 추가합니다:
   - encar.com(PC) 또는 모바일 웹(car.encar.com)에서 국산/수입/친환경 등 원하는 카테고리로 이동해 제조사·모델·연식·가격·주행거리 등을 원하는 대로 필터링합니다.
   - 필터링이 끝난 상태의 **브라우저 주소창 URL 전체**를 복사해 `url` 필드에 붙여넣습니다.
   - 신차대비 비율 상한, 용도변경이력/단순수리/보험이력 정보제공 불가능기간 제외 등은 선택적으로 추가합니다.
3. `poll_interval_sec`(기본 300초, 최소 60초)을 필요하면 조정합니다.
