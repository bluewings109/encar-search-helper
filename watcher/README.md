...
3. `searches`에 감시할 조건을 추가합니다:
   ...
   - 신차대비 비율 상한, 용도변경이력/단순수리 제외 등은 선택적으로 추가합니다.
...
- 신차대비 비율/단순수리/용도변경이력/보험이력 정보제공 불가능기간은 크롬 익스텐션(`../extension`)과 동일한 상세 API 3종(`inspection`, `record`, `options/choice`)을 호출해 계산합니다. 이 조건 중 하나라도 설정된 검색에서만, 신규 매물에 한해 상세 조회를 수행합니다.
